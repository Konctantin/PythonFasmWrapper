...
class FASM:
    def __init__(self, dllPath):
        ...
        try:
            ...
            raise RuntimeError("FASM: can't locate fasm.dll")
        ...
        try:
            ...
            raise RuntimeError("FASM: can't retrive version info")
    ...
    def Assemble(self, source, memorySize = 0x10000, passes = 100, pipes = 0):
        ...
        if state == 2: # ERROR
            ...
            raise RuntimeError('FASM error: ' + self.__FasmErrors[errCode] + " at line " + str(line))

        if not state in [0, 2]: # OK or ERROR
            raise RuntimeError('FASM error: ' + self.__FasmState[res])
